...
<TokenizerEmulator defaultText="No tengas miedo a probar cosas" showEmbeddings={false} />
...
A continuación vienen las **capas transformer**, que son el núcleo del modelo. Cada capa recibe un estado oculto y devuelve otro con la misma forma. Por dentro contienen el mecanismo de auto-atención y una red feed-forward, pero para lo que nos ocupa aquí podemos tratarlas como cajas negras. Lo relevante es que **son el bloque que se repite N veces y concentra la mayor parte de los parámetros del modelo**: un modelo de 1B como TinyLlama tiene 22 capas, uno de 32B puede tener 64. Este es el bloque que más tarda en cargarse y el que justifica toda la técnica.

Después de la última capa viene la **normalización final** (habitualmente RMSNorm), que estabiliza los estados ocultos antes de la proyección final sin cambiar sus dimensiones.

Y por último, el **LM head**, una proyección lineal que lleva los estados ocultos desde la dimensión oculta hasta el tamaño del vocabulario. Su salida es un vector de **logits** con una puntuación por cada token del vocabulario. El token con la puntuación más alta es, a grandes rasgos, la predicción del modelo para la siguiente posición.
...
Estos nombres son los que aparecen como prefijos en el `state_dict` del modelo. Por ejemplo, `model.layers.5.self_attn.q_proj.weight` es la matriz de proyección de las queries en la capa transformer número 5. Chiquito los guarda como una variable de clase<Note label="Nombres de bloques">Chiquito define los nombres como [una variable de clase](https://github.com/elcapo/chiquito/blob/0.1.0/src/chiquito/model.py#L99-L104) para poder adaptarlos fácilmente a arquitecturas que usan otra convención, como ChatGLM, que llama a sus capas `transformer.encoder.layers`.</Note> para poder adaptarse a arquitecturas que usen otros nombres.
...
Chiquito usa este mecanismo para construir, al arrancar, una lista ordenada con todos los bloques del modelo.<Note label="Construcción de la lista de capas">El método [`_build_layers()`](https://github.com/elcapo/chiquito/blob/0.1.0/src/chiquito/model.py#L260-L292) recorre el árbol del modelo para cada uno de los cuatro tipos de bloque y guarda referencias a los módulos junto con sus nombres.</Note> El resultado es una secuencia del tipo `[embed, layer.0, layer.1, ..., norm, lm_head]` que el bucle de inferencia recorre de principio a fin, cargando los pesos de cada bloque justo antes de ejecutarlo y liberándolos después.
