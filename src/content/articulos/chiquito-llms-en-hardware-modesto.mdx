...
Con 67 capas y veinte tokens por generar son más de mil lecturas de disco por respuesta. La buena noticia es que ese disco se puede cambiar por RAM: si tienes algo de memoria libre, puedes precargar por adelantado los pesos que el bucle de inferencia va a pedir a continuación. Chiquito ofrece tres estrategias distintas, todas controladas por un parámetro.<Note label="Interpretación del parámetro">El constructor del modelo interpreta el [parámetro `preload_to_ram`](https://github.com/elcapo/chiquito/blob/0.1.0/src/chiquito/model.py#L129-L135) así: `True` reserva RAM para el modelo entero, `False` desactiva la precarga por completo y un entero *N* precarga una ventana deslizante de *N* capas.</Note>
...
En el modo más agresivo, Chiquito vuelca al arrancar todas las capas del modelo a una caché local en RAM clavada. A partir de ese momento la función que carga pesos a CPU deja de tocar el disco.<Note label="Precarga y despacho">La carga en bloque ocurre durante la [inicialización del modelo](https://github.com/elcapo/chiquito/blob/0.1.0/src/chiquito/model.py#L294-L301) y el [despacho posterior](https://github.com/elcapo/chiquito/blob/0.1.0/src/chiquito/model.py#L311-L316) devuelve el diccionario ya preparado sin tocar el disco.</Note>
...
Aun así, este modo no es tan ingenuo como parece. Mientras la GPU ejecuta la capa *N*, un hilo de fondo ya está leyendo la capa *N+1* desde disco.<Note label="Prefetching en disk-only">El bucle de inferencia lanza un [hilo con un solo trabajador](https://github.com/elcapo/chiquito/blob/0.1.0/src/chiquito/model.py#L468-L475) y, [al final de cada iteración](https://github.com/elcapo/chiquito/blob/0.1.0/src/chiquito/model.py#L484-L491), le pide que vaya trayendo la siguiente capa en paralelo con la ejecución actual.</Note> Es una precarga de un único paso por delante, suficiente para que, si la ejecución de una capa tarda más que leer la siguiente, el coste del disco desaparezca casi por completo.

### La ventana deslizante

El punto intermedio es el más interesante y donde Chiquito aplica un patrón clásico de concurrencia. Cuando tienes RAM libre pero no suficiente para el modelo entero, puedes mantener solo *N* capas en memoria al mismo tiempo y dejar que un hilo de fondo reponga la ventana a medida que el bucle de inferencia la vacía por la cola.<Note label="La caché de ventana">Toda la lógica vive en la clase [`_SlidingWindowCache`](https://github.com/elcapo/chiquito/blob/0.1.0/src/chiquito/model.py#L27-L94), que sincroniza productor y consumidor con una [`threading.Condition`](https://docs.python.org/3/library/threading.html#threading.Condition): un *lock* acompañado de un mecanismo de `wait`/`notify`.</Note> Es un **patrón productor-consumidor** de manual:
...
Hay un detalle técnico pequeño pero crítico en el hilo de fondo: **la lectura del archivo se hace fuera del bloqueo**.<Note label="Lectura fuera del lock">La [carga de una capa](https://github.com/elcapo/chiquito/blob/0.1.0/src/chiquito/model.py#L78-L94) sólo bloquea para comprobar cuántos huecos hay libres y elegir la siguiente capa a cargar. Una vez decidido, termina el bloqueo e inicia la carga de la capa que corresponda; cuando el tensor ya está en memoria vuelve a bloquear por un instante para escribirlo en el diccionario de la caché y despertar al consumidor.</Note> De otra forma el consumidor se bloquearía durante cada lectura de disco y toda la gracia de tener un hilo de fondo se perdería.

La caché se destruye y se vuelve a crear en cada paso del bucle generativo.<Note label="Reinicio por token">Al [inicio de cada inferencia](https://github.com/elcapo/chiquito/blob/0.1.0/src/chiquito/model.py#L303-L309) se detiene la caché anterior y se arranca una nueva desde cero. La ventana solo tiene sentido dentro de una misma pasada por el modelo: al llegar al último bloque, el hilo productor ya no tiene nada que hacer.</Note> Es decir: la ventana no persiste entre tokens. Se construye, recorre las *M* capas durante ese paso y se desmonta al llegar a la última. En la siguiente iteración, cuando toca generar el siguiente token, vuelve a empezar el warmup desde cero.
...
Chiquito no aporta nada original en este frente: se apoya en **bitsandbytes**, que es la librería estándar para esto.<Note label="Bits and bytes">[bitsandbytes](https://github.com/bitsandbytes-foundation/bitsandbytes) ofrece dos modos de cuantización al vuelo: **8 bits** y **4 bits** y se encarga de sustituir las capas lineales del modelo por variantes cuantizadas que convierten el tensor en el momento en que se mueve a GPU.</Note>
...
Si has llegado hasta aquí con ganas de jugar, clona Chiquito,<Note label="Github de Chiquito">Puedes encontrar las fuentes de **Chiquito** en [elcapo/chiquito](https://github.com/elcapo/chiquito).</Note> apúntalo a un modelo que no te quepa en la GPU y échale paciencia.
