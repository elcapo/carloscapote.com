...
En su forma más básica, se usan así:
...
### Modelos fragmentados y el mapa de pesos

Un modelo pequeño puede caber en un único archivo pero los modelos grandes se fragmentan en varios archivos. Cuando un modelo está fragmentado, existe un archivo índice llamado `model.safetensors.index.json` que mapea cada parámetro al fragmento que lo contiene:
...
HuggingFace proporciona `snapshot_download()` para descargar los archivos de un modelo a una caché local:
...
Las llamadas posteriores con el mismo identificador devuelven la ruta cacheada sin volver a descargar nada. Chiquito envuelve esta lógica en una función que primero comprueba si la ruta es un directorio local con archivos de modelo y, solo si no lo es, descarga desde el Hub.<Note label="Resolución de la ruta del modelo">La función [`resolve_model_path()`](https://github.com/elcapo/chiquito/blob/0.1.0/src/chiquito/utils.py#L33-L45) intenta usar la ruta como directorio local antes de recurrir a `snapshot_download()`.</Note>
