...
project: "Chiquito"
projectUrl: "https://github.com/estudio-hawara/chiquito"
draft: true
---
...
- Para contar las capas y construir una lista con sus nombres.<Note label="Contexto">Cuando se [carga la configuración](https://github.com/estudio-hawara/chiquito/blob/0.1.0/src/chiquito/model.py#L167-L189), se crea el modelo en el dispositivo `meta` y se recorre el árbol de módulos para contar cuántas capas tiene.</Note>
- Para detectar la arquitectura del modelo y saber qué clase instanciar.<Note label="Contexto">El campo `config.architectures[0]` se usa para [seleccionar automáticamente la clase](https://github.com/estudio-hawara/chiquito/blob/0.1.0/src/chiquito/auto_model.py#L34) de modelo correcta.</Note>
...
Chiquito carga el tokenizador una sola vez durante la inicialización<Note label="Contexto">Al iniciarse, Chiquito [carga el tokenizador](https://github.com/estudio-hawara/chiquito/blob/0.1.0/src/chiquito/model.py#L150-L152) y lo deja accesible como `model.tokenizer`.</Note> y lo expone para que puedas codificar tus prompts y decodificar las salidas del modelo. Una vez convertido el texto en identificadores, necesitamos los pesos del modelo para procesarlos. Y esos pesos duermen en archivos.
...
La primera vez que llamas al método los pesos del modelo se descargan a una caché local y el método devuelve la ruta en la que los descargó. Las siguientes llamadas devuelven la misma ruta sin volver a descargar nada.<Note label="Contexto">Durante la carga del modelo, Chiquito [intenta usar la ruta como directorio local](https://github.com/estudio-hawara/chiquito/blob/0.1.0/src/chiquito/utils.py#L33-L45) antes de descargarlo.</Note>
...
Estos nombres son los que aparecen como prefijos en el mapa de pesos del modelo.<Note label="Contexto">Chiquito define los nombres como [una variable de clase](https://github.com/estudio-hawara/chiquito/blob/0.1.0/src/chiquito/model.py#L99-L104) para poder adaptarlos fácilmente a arquitecturas que usan otra convención.</Note>

### Recorrer el árbol de módulos

Una vez creado el modelo sobre el dispositivo meta, teniendo en cuenta que tenemos los nombres de cada uno de sus módulos, podemos recorrerlo en un bucle de principio a fin: <Note label="Contexto">Chiquito usa este mecanismo para construir, al [arrancar](https://github.com/estudio-hawara/chiquito/blob/0.1.0/src/chiquito/model.py#L260-L292), una lista ordenada con todos los bloques del modelo.</Note>
...
Lo relevante para Chiquito es algo mucho más práctico: estas posiciones se calculan una sola vez al principio de cada inferencia y se reutilizan en todas las capas. Además, el módulo que las calcula no es un parámetro entrenado del modelo sino una tabla derivada de la configuración, así que cabe siempre en memoria y no necesita cargarse y descargarse capa por capa como el resto de los pesos.<Note label="Contexto">Chiquito [calcula las embeddings posicionales](https://github.com/estudio-hawara/chiquito/blob/0.1.0/src/chiquito/model.py#L340-L346) una vez al principio de cada paso de inferencia y las reutiliza en todas las capas.</Note>
...
Lo que Chiquito hace, la primera vez que trabaja con un modelo, es reorganizar sus pesos en un formato que le resulte cómodo: crea un directorio nuevo y **escribe dentro un archivo Safetensors por cada capa del transformer**.<Note label="Contexto">La lógica de esta partición de modelos vive en [`splitter.py`](https://github.com/estudio-hawara/chiquito/blob/0.1.0/src/chiquito/splitter.py).</Note> Así, durante la ejecución del modelo, cargar una capa concreta es abrir un único archivo con solo los pesos que esa capa necesita y ninguno de más.
...
En el modo más agresivo, Chiquito vuelca al arrancar todas las capas del modelo a una caché local en RAM. A partir de ese momento la función que carga pesos a CPU deja de tocar el disco.<Note label="Contexto">La carga en bloque ocurre durante la [inicialización del modelo](https://github.com/estudio-hawara/chiquito/blob/0.1.0/src/chiquito/model.py#L294-L301) y el [despacho posterior](https://github.com/estudio-hawara/chiquito/blob/0.1.0/src/chiquito/model.py#L311-L316) devuelve el diccionario ya preparado sin tocar el disco.</Note>
...
Aun así, este modo no es tan ingenuo como parece. Mientras la GPU ejecuta la capa *N*, un hilo de fondo ya está leyendo la capa *N+1* desde disco.<Note label="Contexto">El bucle de inferencia lanza un [hilo con un solo trabajador](https://github.com/estudio-hawara/chiquito/blob/0.1.0/src/chiquito/model.py#L468-L475) y, [al final de cada iteración](https://github.com/estudio-hawara/chiquito/blob/0.1.0/src/chiquito/model.py#L484-L491), le pide que vaya trayendo la siguiente capa en paralelo con la ejecución actual.</Note> Es una precarga de un único paso por delante, suficiente para que, si la ejecución de una capa tarda más que leer la siguiente, el coste del disco desaparezca casi por completo.

### La ventana deslizante

El punto intermedio es el más interesante y donde Chiquito aplica un patrón clásico de concurrencia. Cuando tienes RAM libre pero no suficiente para el modelo entero, puedes mantener solo *N* capas en memoria al mismo tiempo y dejar que un hilo de fondo reponga la ventana a medida que el bucle de inferencia la vacía por la cola.<Note label="Contexto">Toda la lógica vive en una [clase](https://github.com/estudio-hawara/chiquito/blob/0.1.0/src/chiquito/model.py#L27-L94), que sincroniza productor y consumidor.</Note>
...
> La aritmética es la que cabría esperar. En fp16 cada parámetro ocupa 2 bytes; en 8 bits, 1 byte; en 4 bits, medio byte. **Pasar a 4 bits reduce cuatro veces el coste de transferencia**, que es el que manda.<Note label="Cuantización">Durante la [inicialización del modelo](https://github.com/estudio-hawara/chiquito/blob/0.1.0/src/chiquito/model.py#L211-L237), si el usuario pide cuantización, se crea un `BitsAndBytesConfig` y se deja que `bitsandbytes` reemplace las capas lineales.</Note>
...
> Si has llegado hasta aquí con ganas de jugar, clona Chiquito,<Note label="Contexto">Puedes encontrar las fuentes de **Chiquito** en [elcapo/chiquito](https://github.com/estudio-hawara/chiquito).</Note> apúntalo a un modelo que no te quepa en la GPU y échale paciencia.
